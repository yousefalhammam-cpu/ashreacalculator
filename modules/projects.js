(function () {
  ...
  function clone(obj, fallback){ try { return JSON.parse(JSON.stringify(obj)); } catch(e){ return fallback; } }
  function toastMsg(msg){ if (typeof toast === 'function') toast(msg); }
  function nowIso(){ return new Date().toISOString(); }
  function uid(){ return 'prj_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,8); }
  ...
  function findProjectIndexByName(name, list){
    ...
    for(var i=0;i<list.length;i++){
      if(normalizeName(list[i].name) === n) return i;
    }
    return -1;
  }
  ...
  function buildProjectSummary(snapshot){
    ...
    var totalBtu = 0, totalCfm = 0, totalTr = 0, grand = 0;
  ...
  function captureProjectState(){
    return {
      name: (G('quote-project') && G('quote-project').value || '').trim(),
      quoteNo: (G('quote-no') && G('quote-no').value || '').trim(),
  ...
  function createProjectCard(project){
    ...
    return ''
      + '<div style="background:var(--s2);border:1px solid var(--b);border-radius:14px;padding:12px;margin-bottom:10px;">'
  ...
  function renderProjects(){
    ...
    if(!all.length){
      listEl.innerHTML = '<div class="qi-empty" style="margin-top:12px;">' + (isAr() ? 'لا توجد مشاريع محفوظة بعد' : 'No saved projects yet') + '</div>';
      return;
    }
